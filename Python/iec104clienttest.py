...
def sendCommandAuto(myClient):
    ...
    psNewValue.tQuality = eIEC870QualityFlags.GD
    value = random.uniform(10.0, 200.0)  # Random float giá trị
    ...
    if i16ErrorCode == 0:
        print(f"[AUTO-CMD] Sent IOA {psDAID.u32IOA} Value {value:.2f}")
    else:
        print(f"[AUTO-CMD] Send Failed - {errorcodestring(i16ErrorCode)}, {errorvaluestring(tErrorValue)}")
...
def main():
    ...
    while(True):
        ...
        arraypointer[0].ai8DestinationIPAddress="192.168.3.110".encode('utf-8')  #iec 104 server ip address
        ...
        try:
            while True:
                time.sleep(5)
                sendCommandAuto(myClient)  # Tự động gửi lệnh điều khiển
