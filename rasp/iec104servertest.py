...
def send_led_control(serverID, ioa, value):
    ...
    psDAID.eTypeID = eIEC870TypeID.C_SE_TC_1  # Control setpoint float
    psDAID.u32IOA = ioa
    ...
    psNewValue.sTimeStamp.u8DayoftheWeek = (now.tm_wday + 1) % 7

    i16ErrorCode = iec104_lib.IEC104ControlOperate(serverID,
                        ctypes.byref(psDAID),
                        ctypes.byref(psNewValue),
                        None,
                        ctypes.byref(tErrorValue))

    print(f"[COMMAND] Gửi LED IOA={ioa}, Value={value} → Status: {i16ErrorCode}")
...
def cbWrite(u16ObjectId, psIEC104DataAttributeID, psIEC104DataAttributeData, psIEC104WriteParameters, ptErrorValue):
    ...
    if IOA == 100 and data_type == eDataTypes.FLOAT32_DATA:
        # Đọc dữ liệu khoảng cách
        data = ctypes.string_at(psIEC104DataAttributeData.contents.pvData, 4)
        distance = struct.unpack('f', data)[0]
        print(f"[SENSOR] Khoảng cách: {distance:.2f} cm")
        ...
        # Gửi điều khiển LED tương ứng về client
        send_led_control(u16ObjectId, target_ioa, 1.0)  # bật
        # Tắt các LED còn lại
        for ioa in [200, 201, 202]:
            if ioa != target_ioa:
                send_led_control(u16ObjectId, ioa, 0.0)  # tắt
...
def main():
    ...
    while(True):
        ...
        sIEC104Config.sServerSet.sServerConParameters.ai8SourceIPAddress = "192.168.3.110".encode('utf-8') # own server bind address
